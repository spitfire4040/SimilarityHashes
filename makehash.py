...
class Nilsimsa(object):
    ...
    def tran3(self, a, b, c, n):
        ...
        # print(((TRAN[(a+n)&255]^TRAN[b]*(n+n+1))+TRAN[(c)^TRAN[n]])&255)
...
def main(argv):
    ...
    with open(base, 'rb') as pcap:
        ...
        for x in range(0, len(hashed)-1, 2):
            ...
            dec = int(hexdigit, 16)
            hashes.append(str(dec))

    print(hashes)


    # with open('/home/jay/Desktop/per_packet/light-1/light-1_hashes','w') as outfile:
    # for item in hashes:
    #     print(','.join(item) + '\n')
